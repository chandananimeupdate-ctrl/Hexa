...
def normalize_lookup_text(text: str, bot_username: str = "") -> str:
    text = (text or "").strip().lower()
    if bot_username:
        text = text.replace(f"@{bot_username.lower()}", "")
    text = re.sub(r"^[!./#]+", "", text)
    text = re.sub(r"\s+", " ", text).strip()
    for prefix in ("move ", "moves ", "nature ", "nat ", "narutre ", "pokemon ", "poke ", "data "):
        if text.startswith(prefix):
            text = text[len(prefix):].strip()
            break
    for suffix in (" move", " moves", " nature", " nat", " narutre", " pokemon", " poke", " data"):
...
def api_name(text: str) -> str:
    return normalize_lookup_text(text).replace(" ", "-").replace(".", "").replace("'", "")
...
def build_data_text(poke: dict, species: dict) -> str:
    ...
    def stat_bar(val):
        filled = max(1, min(6, int((val / 255) * 6) + 1))
    ...
    for key in stat_keys:
        ...
        bar = stat_bar(val)
        lbl = stat_labels[key]
        stat_lines.append(f"`{lbl}: {val} ({lo} - {hi}) {bar}`")
    text = (
        f"*{name}* {' '.join(get_type_emoji(t) for t in types)}\n\n"
        f"○ *REGION* : {region_gen or 'Unknown'}\n"
        f"● *TYPES* : {type_str}\n"
        f"○ *RARITY* : {rarity}\n"
        f"● *CATCH RATE* : {catch_rate} ({catch_pct}%)\n\n"
        f"○ *POKEDEX ID* : {poke_id}\n"
        f"● *ABILITIES* : {', '.join(abilities) or 'None'}\n"
    )
    if hidden:
        text += f"○ *HIDDEN ABLT* : {hidden}\n"
    else:
        text += "○ *HIDDEN ABLT* : None\n"
    if ev_yield:
...
def main_keyboard(name: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("Moves", callback_data=f"moves_{name}_lvl_0"),
         InlineKeyboardButton("Weakness", callback_data=f"weakness_{name}")],
...
async def data_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_pokemon_data(update, name)


async def send_pokemon_data(update: Update, name: str):
    name = api_name(name)
    if not name:
        return
...
METHOD_BUTTON_LABELS = {"lvl": "Level Up", "tm": "TM", "tutor": "Tutor", "egg": "Egg"}
...
async def send_moves(query, name: str, method: str, page: int):
    ...
    tab_row = [
        InlineKeyboardButton(
            f"{'* ' if m == method else ''}{METHOD_BUTTON_LABELS[m]}",
    ...
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton("Prev", callback_data=f"moves_{name}_{method}_{page-1}"))
    nav.append(InlineKeyboardButton(f"{page+1}/{total_pages}", callback_data=f"moves_{name}_{method}_{page}"))
    if page < total_pages - 1:
        nav.append(InlineKeyboardButton("Next", callback_data=f"moves_{name}_{method}_{page+1}"))
    rows = [tab_row]
    if nav:
        rows.append(nav)
    rows.append([InlineKeyboardButton("Back", callback_data=f"backinfo_{name}")])
...
async def send_weakness_inline(query, name: str):
    ...
    text = "\n".join(lines)
    keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("Back", callback_data=f"backinfo_{name}")]])
...
async def send_evolutions(query, name: str):
    ...
    if forms:
        ...
        lines.append("")
        form_buttons = [
            InlineKeyboardButton(f"{f['label']}", callback_data=f"formdata_{name}_{f['name']}")
            for f in forms
        ]
        for i in range(0, len(form_buttons), 2):
            button_rows.append(form_buttons[i:i+2])
    text = "\n".join(lines)
    button_rows.append([InlineKeyboardButton("Back to Info", callback_data=f"backinfo_{name}")])
...
async def send_form_data(query, base_name: str, form_name: str):
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("Moves", callback_data=f"moves_{form_name}_lvl_0"),
         InlineKeyboardButton("Weakness", callback_data=f"weakness_{form_name}")],
        [InlineKeyboardButton("Shiny", callback_data=f"shiny_{form_name}"),
         InlineKeyboardButton("Back", callback_data=f"evos_{base_name}")],
    ])
    if sprite_url:
        try:
            img = req_lib.get(sprite_url, timeout=10)
...
async def send_typechart_inline(query, types: list):
    ...
    try:
        ...
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton(t.title(), callback_data=f"typechart_{t}") for t in valid_types[:6]],
            [InlineKeyboardButton(t.title(), callback_data=f"typechart_{t}") for t in valid_types[6:12]],
            [InlineKeyboardButton(t.title(), callback_data=f"typechart_{t}") for t in valid_types[12:]],
        ])
        await query.edit_message_media(
...
async def search_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        pass
    text = normalize_lookup_text(update.message.text, bot_username)
    if text.startswith("/"):
        return
    if not text:
        return
    if text in NATURE_DATA:
        await send_nature_lookup(update, text)
        return
    for query in [text, api_name(text)]:
        move_data = await get_move(query)
        if move_data:
            await send_move_lookup(update, move_data)
            return
    result = await get_pokemon(text)
    if result:
        await send_pokemon_data(update, text)


async def send_nature_lookup(update: Update, nature_name: str):
    nat = NATURE_DATA[nature_name]
    if nat["boost"]:
        reply = (
            f"🌿 *Nature: {nature_name.title()}*\n\n"
            f"`Boosts  :` {nat['boost']} (+10%)\n"
            f"`Reduces :` {nat['reduce']} (-10%)\n\n"
            f"_{nat['desc']}_"
        )
    else:
        reply = f"🌿 *Nature: {nature_name.title()}*\n\n_Neutral — no stat changes._"
    await update.message.reply_text(reply, parse_mode=ParseMode.MARKDOWN)


async def send_move_lookup(update: Update, move_data: dict):
    move_name = move_data.get("name", "move").replace("-", " ").title()
